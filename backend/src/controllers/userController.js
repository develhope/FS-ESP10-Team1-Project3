...
const logIn = async (req, res) => {
    try {
        ...
        const passwordCompare = await bcrypt.compare(req.body.password_hash, userData.password_hash);
        if (passwordCompare) {
            const { SECRET } = process.env;
            if (!SECRET) {
              throw new Error("JWT SECRET is not defined in environment variables");
            ...
            const token = jwt.sign(payload, SECRET);
            const generateToken = await userModel.generateToken(userData.user_id, token);
            res.status(200).json({ full_name: userData.full_name, token: token, email: userData.email,
                 password_hash: userData.password_hash, user_id: userData.user_id, full_name: userData.full_name, date_of_birth: userData.date_of_birth });
          } else {
...
module.exports = { getAllUsers, getUser, createUser, logIn, checkIfLoged, logOut, deleteUser };
