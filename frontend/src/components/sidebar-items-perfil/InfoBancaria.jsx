...
function InfoBancaria() {
    ...
    const actualizarInfoBancaria = (event) => {
        event.preventDefault();
        const nuevaCuenta = {
            id: Date.now(),
            numero: numeroCuenta, 
            nombre: nombreCuentaBancaria,
            pais: pais, 
            moneda: monedaDePago 
          };
    ...
    return (
        ...
        <button className="nueva-cuenta" onClick={() => mostrarForm ? setMostrarForm(false) : setMostrarForm(true)}>Añadir cuenta</button>
         <form className={mostrarForm ? "mostrar-form" : "no-mostrar-form"} onSubmit={actualizarInfoBancaria}>
            ...
            <div className="cuentasBancarias">
              <ul>
                {cuentasBancarias.map((cuenta) => (
                  ...
                  <button onClick={() => eliminarCuentaBancaria(cuenta)}><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="50" viewBox="0 0 512 512">
          ...
          {cuentaSeleccionada != null ? <h3 className="cuentaActual">Cuenta seleccionada actualmente: {cuentaSeleccionada}</h3> : <h3>no tienes ninguna cuenta seleccionada, por favor, elige una de tus cuentas como cuenta de pago por defecto</h3>}
